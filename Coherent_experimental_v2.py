...
mode_magnitudes = [0.3, 0.7, 0.5, 0.9, 1.0, 0.2]
mode_phases = [0.4, 1.6, 1.0, 0.2, 0.9, 0.5]
...
test_near_path = r"KaelData20260122\0.3exp(0.4j) 0.7exp(1.6j) 0.5exp(1j) 0.9exp(0.2j) 1exp(0.9j) 0.2exp(0.5j).bmp"
test_far_path = r"KaelData20260122\0.3exp(0.4j) 0.7exp(1.6j) 0.5exp(1j) 0.9exp(0.2j) 1exp(0.9j) 0.2exp(0.5j) FF.bmp"
...
def process_experimental_images():
    ...
    sigma_denoise = 1.0
    test_near_aligned = gaussian_filter(test_near_aligned, sigma=sigma_denoise)
    test_far_aligned = median_filter(test_far_aligned, size=3)
    test_far_aligned = gaussian_filter(test_far_aligned, sigma=0.5)
...
def calibrate_far_field_scaling(gaussian_near, gaussian_far):
    ...
    energy_camera = np.sum(g_far_norm**2)
...
def create_gaussian_mask(shape, sigma_r=0.4, order=8):
...
def optimize_coeffs_calibrated(I_near, I_far, basis_modes, dx, dy, init_coeffs, far_field_scale):
    ...
    def loss(coeffs_flat):
        ...
        # Weighted Far field error: 
        # Ignore errors where the experimental intensity is very low (noise floor)
        far_diff = (np.abs(psi_far)**2 - I_far)
        weight = np.where(I_far > 0.02, 1.0, 0.1) # Pay less attention to the 'dark' regions
        far_error = np.sum(weight * (far_diff**2))
...
def propagate_field(psi, z, L, wavelength=633e-9, return_L=False):
    ...
    if pad_total > 0:
        ...
        win = win**0.1
        psi_windowed = psi_resampled * win
        psi_padded = np.pad(psi_windowed, pad_width, mode='constant')
    else:
...
def plot_results(test_near_norm, psi_comparison, psi_retrieved, coeffs_true, coeffs_retrieved, L_actual, test_far_norm, far_field_scale):
    ...
    if KNOWN_BEAM:
        ...
        near_titles = ['Processed Near Field', 'Retrieved Near Field']

    fig1, axes1 = plt.subplots(1, len(near_images), figsize=(5 * len(near_images), 5))
    if len(near_images) == 1:
        axes1 = [axes1]
    ...
    if KNOWN_BEAM:
        ...
        far_titles = ['Processed Far Field', 'Retrieved Far Field']

    fig2, axes2 = plt.subplots(1, len(far_images), figsize=(5 * len(far_images), 5))
    if len(far_images) == 1:
        axes2 = [axes2]
    ...
    rel_phases_ret = np.angle(coeffs_retrieved) - np.angle(coeffs_retrieved[idx_dom_ret])
    ...
    if KNOWN_BEAM:
        ...
        rel_phases_true = np.angle(coeffs_true) - np.angle(coeffs_true[idx_dom_true])
        ...
        ax1.bar(x_pos, retrieved_mags, 0.4, label='Retrieved', alpha=0.7)
        ax1.set_title('Retrieved Magnitudes')
        ax2.bar(x_pos, rel_phases_ret, 0.4, label='Retrieved', alpha=0.7)
        ax2.set_title('Retrieved Relative Phases')
    ...
    if KNOWN_BEAM:
        ...
        fig4, axes4_row = plt.subplots(1, len(propagation_distances), figsize=(20, 4))
        if len(propagation_distances) == 1:
            axes4_row = [axes4_row]
        # Wrap in a 2-row structure so existing indexing axes4[0/1, i] still works for KNOWN_BEAM path;
        # for unknown beam we use axes4_row directly below.

    for i, z in enumerate(propagation_distances):
        psi_ret_prop, L_prop = propagate_field(psi_retrieved, z, L_actual, wavelength, return_L=True)
        I_ret = np.abs(psi_ret_prop)**2
        ret_prop_norm = I_ret / np.max(I_ret)
        extent_val = [-L_prop, L_prop, -L_prop, L_prop]

        if KNOWN_BEAM:
            psi_true_prop = propagate_field(psi_comparison, z, L_actual, wavelength)
            ...
            if np.all(psi_ret_prop == 0) and z > 0:
                axes4_row[i].text(0, 0, "HALTED\n(High Load)", ha='center', color='red')
                continue

            im = axes4_row[i].imshow(ret_prop_norm, cmap='hot', extent=extent_val)
            axes4_row[i].set_title(f'Retrieved: z = {z} m\nFOV: {2*L_prop*1000:.1f} mm')
            axes4_row[i].set_xlabel('x (m)')

            if i > 0:
                axes4_row[i].set_yticks([])
            else:
                axes4_row[i].set_ylabel('y (m)')

    plt.tight_layout()
    
    return fig1, fig2, fig3, fig4

def plot_phase_comparison(psi_comparison, psi_retrieved, L_actual, dx, dy, wavelength=633e-9):
   
    if not propagation_distances:
        return None

    if KNOWN_BEAM:
        fig, axes = plt.subplots(2, len(propagation_distances), figsize=(20, 8))
        if len(propagation_distances) == 1:
            axes = axes.reshape(2, 1)
    else:
        fig, axes_row = plt.subplots(1, len(propagation_distances), figsize=(20, 4))
        if len(propagation_distances) == 1:
            axes_row = [axes_row]
    ...
    for i, z in enumerate(propagation_distances):
        # Propagate retrieved field
        psi_ret_prop, L_prop = propagate_field(psi_retrieved, z, L_actual, wavelength, return_L=True)
        phase_ret = np.angle(psi_ret_prop)
        I_ret = np.abs(psi_ret_prop)**2
        alpha_ret = (I_ret / np.max(I_ret))**0.3
        extent_val = [-L_prop, L_prop, -L_prop, L_prop]

        def plot_with_alpha(ax, phase_data, alpha_data, title):
            # Normalize phase to [0, 1] for colormap
            norm_phase = (phase_data + np.pi) / (2 * np.pi)
            # Get RGB colors from colormap
            rgba = phase_cmap(norm_phase)
            # Apply calculated intensity as the Alpha channel
            rgba[..., 3] = np.clip(alpha_data, 0, 1)
            ax.imshow(rgba, extent=extent_val)
            ax.set_title(title)
        ...
        if KNOWN_BEAM:
            ...
            # CALCULATE FIDELITY
            overlap = np.sum(psi_true_prop * np.conj(psi_ret_prop))
            norm_true = np.sum(np.abs(psi_true_prop)**2)
            norm_ret = np.sum(np.abs(psi_ret_prop)**2)
            fidelity = (np.abs(overlap)**2) / (norm_true * norm_ret + 1e-15)

            phase_true = np.angle(psi_true_prop)
            I_true = np.abs(psi_true_prop)**2
            alpha_true = (I_true / np.max(I_true))**0.3

            # Plot True Phase
            plot_with_alpha(axes[0, i], phase_true, alpha_true, f'True Phase: z = {z} m')
            
            # Plot Retrieved Phase
            plot_with_alpha(axes[1, i], phase_ret, alpha_ret, f'Retrieved Phase\nFidelity: {fidelity:.4f}')
            ...
            plot_with_alpha(axes_row[i], phase_ret, alpha_ret, f'Retrieved Phase: z = {z} m')

            if i == 0:
                axes_row[i].set_ylabel('y (m)')
            else:
                axes_row[i].set_yticks([])
...
def main_coherent():
    ...
    # Resolve conjugate ambiguity by comparing far fields
    ...
    psi_std_far = fftshift(fft2(ifftshift(psi_std))) * far_field_scale
    psi_flip_far = fftshift(fft2(ifftshift(psi_flip))) * far_field_scale
    
    rmse_std = calculate_intensity_rmse(test_far_norm, np.abs(psi_std_far)**2)
    rmse_flip = calculate_intensity_rmse(test_far_norm, np.abs(psi_flip_far)**2)
    ...
    # Align global phase to the comparison coefficients (if known) or to dominant mode
    ...
    print("\nRetrieved Mode Coefficients:")
    print(f"  {'Mode':<10} {'Magnitude':>12} {'Phase (rad)':>12} {'Power (%)':>10}")
    print(f"  {'-'*46}")
    total_power = np.sum(np.abs(coeffs_retrieved)**2)
    for (m, n), c in zip(mode_list, coeffs_retrieved):
        mag = np.abs(c)
        phase = np.angle(c)
        power_pct = 100 * mag**2 / total_power
        print(f"  ({m},{n}){'':<6} {mag:>12.4f} {phase:>12.4f} {power_pct:>9.2f}%")
    ...
    # NEW: Generate phase plot
    fig5 = plot_phase_comparison(psi_comparison, psi_retrieved, L_actual, dx, dy)
