...
mode_magnitudes = [0.3, 0.7, 0.5, 0.9, 1, 0.2]
mode_phases = [0.4, 1.6, 1, 0.2, 0.9, 0.5]
...
def calculate_fidelity(psi1, psi2, dx, dy):
    
    # Calculate the inner product
    overlap = np.sum(psi1 * np.conj(psi2)) * dx * dy
    
    # Calculate norms
    norm1 = np.sum(np.abs(psi1)**2) * dx * dy
    norm2 = np.sum(np.abs(psi2)**2) * dx * dy
    
    fidelity = (np.abs(overlap)**2) / (norm1 * norm2)
